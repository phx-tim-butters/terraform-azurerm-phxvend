...
locals {
  ...
  custom_resource_groups = { for key, group in var.resource_groups : "${group.location}-${group.resource_name}" => {
    resource_name             = group.resource_name
    resource_name_overwrite   = try(group.resource_name_overwrite, false)
  ...
  default_region_resource_groups = { for key, group in local.default_resource_groups : "${var.default_location}-${key}" => {
    resource_name             = key
    resource_name_overwrite   = try(group.resource_name_overwrite, false)
    resource_group_short_name = key
    location                  = try(group.location, var.default_location)
  ...
  // This was a pain in the arse, but the code will take in the regions we're stamping down templated resource groups on, along with the templates resource groups themselves.
  // it'll produce a map that we can merge with the resource groups explicitly defined.
